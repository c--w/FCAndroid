...
/**
 * Creates the turn points. Gets data by parsing the stream of tokens. Or creates default data if no stream is passed in.
 */
class TurnPointManager {
	...
	void boundingBox(float[] a, float[] b) {
		...
		for (int i = 1; i < turnPoints.length; i++) {
			// x min, y min
			if (turnPoints[i].x < a[0])
				a[0] = turnPoints[i].x;
			if (turnPoints[i].y < a[1])
				a[1] = turnPoints[i].y;

			// x max, y max
			if (turnPoints[i].x > b[0])
				b[0] = turnPoints[i].x;
			if (turnPoints[i].y > b[1])
				b[1] = turnPoints[i].y;
		}
	}
}
