...
/**
 * A list of points to fly round when ridge soaring. Usually two points, unless
 * ridge is snakey. NB We use the hill's local coord system.
 * 
 * Also used for roads.
 */
class Circuit {
	...
	int next = 0;
	float[] fallLine;
	boolean closed = true; // flag - do we go back to start after reaching the
							// last point
	...
	/**
	 * Returns the next point on the circuit. Or null if we have reached the end
	 * of the circuit.
	 */
	float[] next() {
		float[] p;
